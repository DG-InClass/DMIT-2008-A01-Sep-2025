...
const renderAstronautListItem = function(astronautData, listElement) {
    let status = astronautData.status.name === 'Active' ? 'Active' : 'Retired';
    if(astronautData.in_space) {
        status += ' | Currently in Space'
    }
    const listItem = `<li href="#" class="list-group-item list-group-item-action">
    <div class="d-flex w-100 justify-content-between">
        <img src="${astronautData.profile_image_thumbnail}" class="rounded float-start" alt="${astronautData.name} thumbnail">    
        <h5 class="mb-1">${astronautData.name} (${status})</h5>
